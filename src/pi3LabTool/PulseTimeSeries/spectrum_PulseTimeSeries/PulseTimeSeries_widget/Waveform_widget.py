...
from spectrum_pulse_time_series import PulseTimeSeries
...
class WaveformPanel(QWidget):
    ...
    def start(self, params):
        ...
        # start device
        self.device.device_name = self.device_name
        # self.device.clk_terminal = self.clk_terminal
        self.device.samplerate = self.sample_rate
        # self.device._frame_size = self.frame_size
        # self.device._frame_num = self.frame_num
        self.device.channel = self.analog_channels
        self.device.input_range = self.adc_voltage_range
        self.device.timeout = self.rw_timeout
        # self.device.external_sample_clock_source = self.external_sample_clock_source
        self.device._enable_debug = self.enable_debug

        self.device.on_activate()
        self.device.configure(bin_width_s = 1/self.sample_rate, 
    ...
    def stop(self):
        if not self.running:
            return
        # stop nidaq device
        self.device.stop_measure()
        self.device.on_deactivate()
        self.timer.stop()
        self.data.clear()
        self.line.set_data([], [])
    ...
    def _update_plot(self):
        ...
        # get data from nidaq ---
        data, info = self.device.get_data_trace(accumulate = True)
