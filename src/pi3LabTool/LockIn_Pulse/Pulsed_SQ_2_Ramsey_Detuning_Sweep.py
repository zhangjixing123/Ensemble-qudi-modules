...
det_stop = 1e6
det_step = 20e3
det_list = np.arange(det_start,det_stop,det_step)
f_LIA =10e3
CLOCK_FREQ_MHZ = 200
...
sp.pb_start()
#
setTaborCW_Output(freq+det_list[0] , power_MW , 0x168C, 0x1202)
###Measurement###
RATE = 1e3
TIME_OUT = 10
POINTS = 500
...
with Task() as task:
    ...
    for i,det in enumerate(det_list):
        print(det)
        setfreq(int(freq+det), 0x168C, 0x1202)
        sleep(0.3)
        task.start()
...
ax.plot(det_list,data_list)
plt.show()

idling(t_ini,f_LIA)
#saving

save_path = r'C:\Users\pi3-pc12\Desktop\Yuta\Experimental Data\Pulsed-Detuning-Sweep'
