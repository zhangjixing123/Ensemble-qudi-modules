...
__all__ = ('correct_offset_histogram', 'find_highest_peaks', 'estimate_double_peaks',
           'estimate_triple_peaks', 'sort_check_data', 'smooth_data','estimate_multiple_peaks')
...
def estimate_double_peaks(data, x, minimum_distance=None):
    ...
    print('data',data[0])
    print('x',x[0])
    print('minimum',minimum_distance)
    print('the index of peak_indices:',peak_indices)
    print('the number of peak_indices:',len(peak_indices))
...
def estimate_multiple_peaks(data, x, peak_number,  minimum_distance=None):
    # Find peaks along with width and amplitude estimation
    peak_indices, peak_heights, peak_widths = find_highest_peaks(data,
                                                                 peak_count=peak_number,
                                                                 width=minimum_distance,
                                                                 height=0.05 * max(data))
    x_spacing = min(abs(np.ediff1d(x)))
    x_span = abs(x[-1] - x[0])
    ...
    if len(peak_indices) < peak_number:
        # make a wild guess
        easygui.msgbox(msg='cannot rightly caculate all the desired peaks!!',title='Datafitting of ODMR',ok_button='OK',image=None,root=None)
        peak_indices = list()
        for i in range(peak_number):
            peak_indices.append(len(x)//peak_number * (i + 1))
        peak_indices = tuple(peak_indices)
        peak_heights = tuple([data_span]*peak_number)
        peak_widths = tuple([x_spacing * 10]*peak_number)

    estimate = {'height': np.asarray(peak_heights),
                'fwhm'  : np.asarray(peak_widths) * x_spacing,
                'center': np.asarray(x[np.asarray(peak_indices)])}
    limits = {'height': ((0, 2 * data_span),) * peak_number,
              'fwhm'  : ((x_spacing, x_span),) * peak_number,
              'center': ((min(x) - x_span / 2, max(x) + x_span / 2),) * peak_number}
    return estimate, limits
