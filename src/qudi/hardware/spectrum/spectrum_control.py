# spectrum_control_temp.py
# Control and data acquisition for Spectrum digitizers using state machine
# Supports initialization, starting/stopping acquisition, and data averaging
...
from .plot_server import RealtimePlotProcess
...
class Spectrum_State_Trans(object):
    ...
    def __init__(self, pipe1, pipe2, state, enable_debug=False):
        ...
        # 启动绘图进程
        self.plotter = RealtimePlotProcess(self.input_queue, buffer_len=30000, window_len=10000)
        self.plotter.start()
    ...
    def INIT_begin(self):
        ...
        # default parameters -------------------------------------
        self.qwBufferSize = uint64(KILO_B(parameters[0])*20)
        ...
        self.num_gate = int(parameters[2] * 1024 / 2 / parameters[1])
        print('gate number:', self.num_gate)
        self.speed_ratio = max([i for i in range(1, self.num_gate) if self.num_gate % i == 0]) # The greatest common factor other than itself
        self.ReadSize = int32(KILO_B(int(self.lSegmentSize * 2 * self.speed_ratio / 1024))) 
    ...
    def SS_begin(self):
        ...
        start_ptr = addressof(self.pvBuffer.contents) # start address of the buffer
        end_ptr = start_ptr + self.qwBufferSize.value # end address of the buffer
        compare = 0
            ...
            while not self.stop_symbol and self._state.value:
                ...
                while self.qwTotalMem.value < self.qwToTransfer.value:
                    ...
                    if self.dwError != ERR_OK:
                        ...
                        if self.lAvailUser.value >= self.lNotifySize.value * self.speed_ratio:
                            self.qwTotalMem.value += self.lNotifySize.value * self.speed_ratio
                            ...
                            cur_ptr = addressof(self.pvBuffer.contents) + self.lPCPos.value # current pointer position
                            # check if reading operation will meet the end address of the buffer
                            if cur_ptr + self.ReadSize.value > end_ptr:
                                # if yes, read date to end address of buffer first, then read the rest of required date from the start address of the buffer
                                pnData_first = cast(addressof(self.pvBuffer.contents) + self.lPCPos.value, ptr16)
                                pnData_after = cast(addressof(self.pvBuffer.contents), ptr16)
                                num_first = (end_ptr - cur_ptr + 1)//2
                                temp_first = np.array(pnData_first[:num_first])
                                num_rest = self.ReadSize.value//2 - len(temp_first)
                                temp_after = np.array(pnData_after[:num_rest])
                                temp = np.concatenate((temp_first, temp_after))
                                # print('MODE 1')
                            else:
                                pnData = cast(addressof(self.pvBuffer.contents) + self.lPCPos.value, ptr16)  # cast to pointer to 16bit integer
                                temp = np.array(pnData[:self.ReadSize.value//2])
                                # print('MODE 2')

                            # pnData = cast(addressof(self.pvBuffer.contents) + self.lPCPos.value, ptr16)  # cast to pointer to 16bit integer
  
                            # temp = np.array(pnData[:self.lNotifySize.value * self.speed_ratio // 2])
                            temp = np.vstack(np.array_split(temp, self.speed_ratio))
                            if not len(store_data[0]):
                                store_data = temp
                            else:
                                store_data = np.append(store_data, temp, axis=0)
                            spcm_dwSetParam_i32(self.hCard, SPC_DATA_AVAIL_CARD_LEN, self.ReadSize)
                            
                            spcm_dwGetParam_i32(self.hCard, SPC_DATA_AVAIL_USER_LEN, byref(self.lAvailUser))
                            spcm_dwGetParam_i32(self.hCard, SPC_DATA_AVAIL_USER_POS, byref(self.lPCPos))

                            # plot_data = [self.qwTotalMem.value, self.qwToTransfer.value, self.lAvailUser.value, self.lPCPos.value]
                            plot_data = [self.qwTotalMem.value, self.qwToTransfer.value, self.lAvailUser.value, self.lPCPos.value]
                            self.input_queue.put(plot_data)

                            # if self.lPCPos.value < compare:
                            #     print(self.qwBufferSize.value - compare - self.lNotifySize.value)
                            # else:
                            #     compare = self.lPCPos.value
...
def average_func(pipe2, pipe3, cmd, Vmax=5000):
    ...
    while True:
        ...
        if cmd.value == -1:
            break
        while cmd.value:
            # print('Receiving data from digitizer...', end='', flush=True)
            if pipe2.poll(20):
                cur_data = pipe2.recv()
            else:
            ...
            if cur_data is None:
                # print('receive no data!', end='', flush=True)
                break 
            sweep = pipe2.recv() - 1
            # print('Update # of sweep:', sweep, flush=True)

            # print('Averaging accumulated data...', end='', flush=True)
            ...
            # print('Sending the last data to Pulsed GUI...', end='', flush=True)
            if cmd.value == 3:
                if len(cur_data) == len(store_data):
            ...
            # print('Sending accumulated data to Pulsed GUI...', end='', flush=True)
            if cmd.value == 2:
