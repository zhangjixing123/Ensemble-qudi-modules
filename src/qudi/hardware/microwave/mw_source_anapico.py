...
class MicrowaveAnaPicoAPSin(MicrowaveInterface):
    ...
    def on_activate(self):
        ...
        if self._model == 'APSIN3000-HC':
            freq_limits = (9e3, 3.3e6)
            power_limits = (-30, 13)
            scan_size_limits = (2,1000) # The pyvisa throws an error if the size is to large.... despite the actual hardawre limitation
        else:
            freq_limits = (9e3, 3.3e6)
            power_limits = (-30, 13)
        ...
        # TODO set all activation vals ..... why again?
...
# ------------- Inputs -----------------------------------
    ...
    def configure_scan(self, frequencies, power, mode):
...
# -------------- Helpers ---------------------------------
    ...
    def _write_list(self):
        ...
        self._command_wait(f':LIST:POW {self.scan_power:f}')
