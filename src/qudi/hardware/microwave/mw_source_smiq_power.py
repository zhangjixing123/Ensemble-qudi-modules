...
class MicrowaveSmiq_power(MicrowaveInterface):
    ...
    def _write_list(self):
        ...
        self._device.write(f':LIST:POW {freq_str}')
        self._device.write('*WAI')
        self._device.write(':LIST:MODE STEP')
        self._device.write('*WAI')

        # Set list power
        self._device.write(f':LIST:FREQ {self._scan_power:f}')
        self._device.write('*WAI')
