...
pulsed_spectrum_finite_sampling_input:
    module.Class: 'spectrum_spincore_finite_sampling_input.PulsedFiniteSamplingInput'
    connect:
        digitizer:          'spectrum_fast_sampling'
        pulser:             'pulseblaster'
    options:
        channel_aom:        1
        channel_mw:         3
...
class PulsedFiniteSamplingInput(FiniteSamplingInputInterface):
    ...
    def start_buffered_acquisition(self, reset=True):
    ...
    def acquire_frame(self, reset=True):
        ...
        with self._thread_lock:
            ...
            self.start_buffered_acquisition(reset=reset)
    ...
    def _init_digitizer(self):
        ...
        period = 1./self.sample_rate
        if self.mode in ['cw', 'cw_awg']:
            record_length_s = period*0.7
            bin_width_s = record_length_s/(2**13)
            number_of_gates = self.frame_size*2
        if self.mode in ['pulsed', 'pulsed_awg']:
            record_length_s = self._t_readout
            bin_width_s = record_length_s/512
