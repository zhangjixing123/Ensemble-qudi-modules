...
#CARE! This is the changed version just for the use of the KIM101 piezo stage from Thorlabs!
...
class NiScanningProbeInterfuse(ScanningProbeInterface):
    ...
    ni_scanning_probe:
        ...
        connect:
            ...
            piezo_motor: 'piezo_motor'
        options:
            controller_channels:
                (1, 2)      #channels with which the x and y axis motors are linked (first number in tuple is x, second is y)
            ni_channel_mapping:
                x: 'x'
                y: 'y'
                APD1: 'PFI8'
    ...
    __max_move_velocity = ConfigOption(name='maximum_move_velocity')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._current_scan_ranges = [(-1e-2, 1e-2), (-1e-2, 1e-2)]
        self._current_scan_axes = self._controller_channels
...
#
    ...
    def _toggle_ao_setpoint_channels(self, enable: bool) -> None:
        ...
        for channel in self._controller_channels:
            self._kim101_piezo.controller.enable_channels(channel)

    @property
    def _ao_setpoint_channels_active(self) -> bool:
        mapped_channels = set(self._controller_channels.values())
        return all(
            state for ch, state in self._kim101_piezo.activity_states.items() if ch in mapped_channels
        )
    ...
    def on_deactivate(self):
        ...
        if self._kim101_piezo.controller.is_moving:
    ...
    def get_constraints(self):
        ...
        """
        return self._kim101_piezo.get_constraints()
    ...
    def move_absolute(self, position, velocity=None, blocking=False):
        ...
        if velocity != None:
            self._kim101_piezo.set_velocity(velocity)	
        self._kim101_piezo.move_abs(position)
        return
    ...
    def __wait_on_move_done(self):
        ...
        #except:
        #    self.log.exception("")
        return self._kim101_piezo.controller.wait_move()
    ...
    def move_relative(self, distance, velocity=None, blocking=False):
        ...
        if velocity != None:
            self._kim101_piezo.set_velocity(velocity)	
        self._kim101_piezo.move_rel(distance)
        return
    ...
    def get_target(self):
        ...
        #else:
        return self._kim101_piezo.get_pos()
    ...
    def emergency_stop(self):
        ...
        return self._kim101_piezo.abort()
    ...
    def _initialize_ni_scan_arrays(self, scan_data):
        ...
        if scan_data.scan_dimension == 1:
            ...
            horizontal = np.linspace((axis, scan_data.scan_range[0]),
                                     horizontal_resolution)

            horizontal_return_line = np.linspace((axis, scan_data.scan_range[0][1]),
                                                 (axis, scan_data.scan_range[0][0]),
                                                 self.__backwards_line_resolution)
            ...
            position_dict = {self._ni_channel_mapping[axis]: horizontal_single_line}
            ...
            horizontal = np.linspace((horizontal_axis, scan_data.scan_range[0]),
                                     horizontal_resolution)

            horizontal_return_line = np.linspace((horizontal_axis, scan_data.scan_range[0][1]),
                                                 (horizontal_axis, scan_data.scan_range[0][0]),
                                                 self.__backwards_line_resolution)
            ...
            vertical = np.linspace((vertical_axis, scan_data.scan_range[1]),
                                   vertical_resolution)
        ...
        try:
            ...
            with self._thread_lock_cursor:
                ...
                if not stop_loop:
                    ...
                    self._kim101_piezo.move_abs = new_pos  #new_voltage
