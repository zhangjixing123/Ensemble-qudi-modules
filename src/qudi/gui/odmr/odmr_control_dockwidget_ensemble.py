...
class OdmrPulsedControlDockWidget(AdvancedDockWidget):
    sigModeChanged = QtCore.Signal(str)
    sigRunsPerPeriodChanged = QtCore.Signal(int)
    sigTPreChanged = QtCore.Signal(float)
    sigTPiChanged = QtCore.Signal(float)
    sigTDelayChanged = QtCore.Signal(float)
    sigTReadoutChanged = QtCore.Signal(float)
    
    def __init__(
            self, *args,
            mode = 'cw', t_pre = 2.0,
            runs_per_period = 10, t_pi = 1.e-6,
            t_delay = 2.e-6, t_readout = 20.e-5,
            **kwargs
        ):
        super().__init__(*args, **kwargs)
        self.setWindowTitle('ODMR Pulsed Control')
        ...
        self.mode_menu.addItems(['cw', 'cw_awg', 'pulsed', 'pulsed_awg'])
        self.mode_menu.setCurrentText(mode)
        self.mode_menu.currentTextChanged.connect(self._mode_changed_cb)
        h_layout.addWidget(self.mode_menu)

        label = QtWidgets.QLabel('t_pre:')
        label.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        h_layout.addWidget(label)

        self.TPre_spinbox = ScienDSpinBox()
        self.TPre_spinbox.setMinimumWidth(self._min_spinbox_width)
        self.TPre_spinbox.setMinimum(0)
        self.TPre_spinbox.setDecimals(1)
        self.TPre_spinbox.setSuffix('s')
        self.TPre_spinbox.setValue(t_pre)
        self.TPre_spinbox.valueChanged.connect(self._TPre_changed_cb)
        h_layout.addWidget(self.TPre_spinbox)

        main_layout.addLayout(h_layout)

        # create sequence parameters group box
        group_box = QtWidgets.QGroupBox('Pulsed ODMR Parameters')
        layout = QtWidgets.QGridLayout()
        layout.setColumnStretch(1, 1)
        layout.setColumnStretch(3, 1)
        group_box.setLayout(layout)

        label = QtWidgets.QLabel('Runs per period:')
        label.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        layout.addWidget(label, 0, 0)

        self.RunsPerPeriod_spinbox = QtWidgets.QSpinBox()
        self.RunsPerPeriod_spinbox.setMinimumWidth(self._min_spinbox_width)
        self.RunsPerPeriod_spinbox.setMinimum(1)
        self.RunsPerPeriod_spinbox.setValue(runs_per_period)
        self.RunsPerPeriod_spinbox.valueChanged.connect(self._RunsPerPeriod_changed_cb)
        layout.addWidget(self.RunsPerPeriod_spinbox, 0, 1)

        label = QtWidgets.QLabel('t_pi:')
        label.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        layout.addWidget(label, 0, 2)
        self.TPi_spinbox = ScienDSpinBox()
        self.TPi_spinbox.setMinimumWidth(self._min_spinbox_width)
        self.TPi_spinbox.setMinimum(0)
        self.TPi_spinbox.setDecimals(1)
        self.TPi_spinbox.setSuffix('s')
        self.TPi_spinbox.setValue(t_pi)
        self.TPi_spinbox.valueChanged.connect(self._TPi_changed_cb)
        layout.addWidget(self.TPi_spinbox, 0, 3)

        label = QtWidgets.QLabel('t_delay:')
        label.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        layout.addWidget(label, 1, 0)
        self.TDelay_spinbox = ScienDSpinBox()
        self.TDelay_spinbox.setMinimumWidth(self._min_spinbox_width)
        self.TDelay_spinbox.setMinimum(0)
        self.TDelay_spinbox.setDecimals(1)
        self.TDelay_spinbox.setSuffix('s')
        self.TDelay_spinbox.setValue(t_delay)
        self.TDelay_spinbox.valueChanged.connect(self._TDelay_changed_cb)
        layout.addWidget(self.TDelay_spinbox, 1, 1)

        label = QtWidgets.QLabel('t_readout:')
        label.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
        layout.addWidget(label, 1, 2)
        self.TReadout_spinbox = ScienDSpinBox()
        self.TReadout_spinbox.setMinimumWidth(self._min_spinbox_width)
        self.TReadout_spinbox.setMinimum(0)
        self.TReadout_spinbox.setDecimals(1)
        self.TReadout_spinbox.setSuffix('s')
        self.TReadout_spinbox.setValue(t_readout)
        self.TReadout_spinbox.valueChanged.connect(self._TReadout_changed_cb)
        layout.addWidget(self.TReadout_spinbox, 1, 3)
    ...
    @property
    def runs_per_period(self):
        return self.RunsPerPeriod_spinbox.value()

    @property
    def t_pre(self):
        return self.TPre_spinbox.value()
    
    @property
    def t_pi(self):
        return self.TPi_spinbox.value()
    
    @property
    def t_delay(self):
        return self.TDelay_spinbox.value()
    
    @property
    def t_readout(self):
        return self.TReadout_spinbox.value()

    def _mode_changed_cb(self):
        self.sigModeChanged.emit(self.mode_menu.currentText())

    @QtCore.Slot()
    def _RunsPerPeriod_changed_cb(self):
        self.sigRunsPerPeriodChanged.emit(self.RunsPerPeriod_spinbox.value())

    @QtCore.Slot()
    def _TPre_changed_cb(self):
        self.sigTPreChanged.emit(self.TPre_spinbox.value())

    @QtCore.Slot()
    def _TPi_changed_cb(self):
        self.sigTPiChanged.emit(self.TPi_spinbox.value())

    @QtCore.Slot()
    def _TDelay_changed_cb(self):
        self.sigTDelayChanged.emit(self.TDelay_spinbox.value())

    @QtCore.Slot()
    def _TReadout_changed_cb(self):
        self.sigTReadoutChanged.emit(self.TReadout_spinbox.value())
