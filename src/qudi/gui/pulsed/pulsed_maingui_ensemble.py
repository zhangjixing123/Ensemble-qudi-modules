...
class PulseAnalysisTab(QtWidgets.QWidget):
    def __init__(self):
        # Get the path to the *.ui file
        this_dir = os.path.dirname(__file__)
        ui_file = os.path.join(this_dir, 'ui_pulse_analysis_ensemble.ui')
        # Load it
        super().__init__()
        uic.loadUi(ui_file, self)
...
class PulseGeneratorTab(QtWidgets.QWidget):
    def __init__(self):
        # Get the path to the *.ui file
        this_dir = os.path.dirname(__file__)
        ui_file = os.path.join(this_dir, 'ui_pulse_editor_ensemble.ui')
        # Load it
        super().__init__()
        uic.loadUi(ui_file, self)
...
class PulsedMeasurementGui(GuiBase):
    ...
    runBenchmark = extraction_import_path = ConfigOption(name='run_benchmark', default=False)
    ...
    def on_activate(self):
        ...
        if self.runBenchmark and not self.pulsedmasterlogic().sequencegeneratorlogic().has_valid_pg_benchmark():
